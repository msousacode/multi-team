...
public interface FolderProfessionalRepository extends JpaRepository<FolderProfessional, UUID> {
    ...
    @Query(value = "select pf from FolderProfessional pf where pf.folder.id = :folderId and pf.professional.id = :professionalId")
    List<FolderProfessional> findByFolderIdAndProfessionalId(UUID folderId, UUID professionalId);
